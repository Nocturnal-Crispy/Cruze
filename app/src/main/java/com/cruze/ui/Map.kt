 ...
 */
fun MapView.drawRoute(shape: List<LatLon>, color: Int = CruzeColors.RouteLine.toArgb()) {
    if (shape.size < 2) return
    val pts = shape.map { it.geo() }
    val density = resources.displayMetrics.density
    overlays.add(Polyline(this).apply {
        outlinePaint.color = CruzeColors.RouteCasing.toArgb()
...
fun MapView.drawTrack(shape: List<LatLon>) {
    if (shape.size < 2) return
    overlays.add(Polyline(this).apply {
        outlinePaint.color = CruzeColors.TrackLine.toArgb()
...
private fun pinDrawable(ctx: Context, color: Int, label: String?): Drawable {
    ...
    p.color = CruzeColors.RouteCasing.toArgb()
